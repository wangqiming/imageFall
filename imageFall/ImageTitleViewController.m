...
//  imageFall
//
//  Created by wangqiming on 16/3/4.
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i<[_imageArr count]; i++) {
        text = [NSString stringWithFormat:@" %d Auto Layout", i+1];
        for (int j = 0; j < i; j++) {
            text = [NSString stringWithFormat:@"%@ Auto Layout",text];
        }
        [_titleArr addObject:text];
    }

    UINib *nib = [UINib nibWithNibName:NSStringFromClass([ImageTitleCell class]) bundle:nil];
    [self.tableView registerNib:nib forCellReuseIdentifier:NSStringFromClass([ImageTitleCell class])];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ImageTitleCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ImageTitleCell class]) forIndexPath:indexPath];
